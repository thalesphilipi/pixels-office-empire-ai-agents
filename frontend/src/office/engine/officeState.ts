...
export class OfficeState {
  ...
  assembleMeeting(participantsIds: number[]): void {
    if (this.walkableTiles.length === 0) return;
    let sumCol = 0, sumRow = 0;
    for (const t of this.walkableTiles) { sumCol += t.col; sumRow += t.row; }
    const centerCol = Math.floor(sumCol / this.walkableTiles.length);
    const centerRow = Math.floor(sumRow / this.walkableTiles.length);

    // 1. Sort points by distance to center
    const sortedTiles = [...this.walkableTiles].sort((a, b) => {
    ...
    for (const id of participantsIds) {
      ...
      // Unassign from their desk
      if (ch.seatId) {
        const seat = this.seats.get(ch.seatId);
        if (seat) seat.assigned = false;
      }
      ch.seatId = null;
      ch.isActive = false; // Make them idle

      const target = sortedTiles[i % sortedTiles.length];
      i++;
      ...
      if (path.length > 0) {
        ...
        ch.frame = 0;
        ch.frameTimer = 0;
        ch.wanderTimer = 999999; // Stay there for the meeting
      }
    }
  }
  ...
  setAgentActivity(id: number, activity: string, detail?: string): void {
    ...
    if (activity === 'idle' || !icon) {
      ...
      ch.activityIcon = icon
      ch.activityTimer = 8 // Show for 8 seconds
      // Update mood based on activity
      if (activity === 'praying') ch.mood = 'praying'
      else if (activity === 'coding' || activity === 'searching') ch.mood = 'focused'
      else if (activity === 'thinking' || activity === 'thinking_deep' || activity === 'analyzing') ch.mood = 'focused'
      else ch.mood = 'happy'
    }
  }

  /** Show floating chat text above a character */
  showChatText(id: number, text: string): void {
    const ch = this.characters.get(id)
    if (!ch) return
    // Truncate to 60 chars for display
    ch.chatText = text.length > 60 ? text.substring(0, 57) + '...' : text
    ch.chatTextTimer = 6 // Show for 6 seconds
  }

  /** Trigger the investor "God speaks" effect — all agents look up */
  triggerInvestorEffect(): void {
    for (const ch of this.characters.values()) {
      ch.lookingUp = true
      ch.lookUpTimer = 3 // Look up for 3 seconds
      ch.dir = 3 // UP direction
    }
  }
  ...
  update(dt: number): void {
    ...
    for (const ch of this.characters.values()) {
      ...
      // Tick activity icon timer
      if (ch.activityIcon && ch.activityTimer > 0) {
        ch.activityTimer -= dt
      ...
      // Tick chat (floating text) timer
      if (ch.chatText && ch.chatTextTimer > 0) {
        ch.chatTextTimer -= dt
        if (ch.chatTextTimer <= 0) {
          ch.chatText = null
          ch.chatTextTimer = 0
        }
      }

      // Tick look up timer (investor effect)
      if (ch.lookingUp && ch.lookUpTimer > 0) {
        ch.lookUpTimer -= dt
        if (ch.lookUpTimer <= 0) {
          ch.lookingUp = false
          ch.lookUpTimer = 0
