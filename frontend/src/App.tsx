...
function App() {
  ...
  const handleClick = useCallback((agentId: number) => {
    // Open the Info Panel specifically for the clicked agent
    setSelectedAgentInfoId(agentId)
  ...
  return (
    <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden' }}>
      ...
      <BottomToolbar
        ...
        onOpenAgents={() => setActiveModal('agents')}
      ...
      {activeModal === 'vault' && (
        <div style={{ position: 'absolute', inset: '40px', zIndex: 100, background: 'var(--pixel-bg)', border: '4px solid var(--pixel-border)', display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '10px', background: 'var(--pixel-border)', display: 'flex', justifyContent: 'flex-end' }}>
            <button style={{ background: '#ef4444', color: '#fff', border: 'none', padding: '5px 10px', cursor: 'pointer' }} onClick={() => setActiveModal(null)}>X CLOSE</button>
          </div>
          <VaultPanel />
        </div>
      )}
      ...
      {showChat && <ChatPanel chatLog={chatLog} onClose={() => setShowChat(false)} />}
